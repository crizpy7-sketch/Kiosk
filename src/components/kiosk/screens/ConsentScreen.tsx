  ...
  return (
    <KioskScreen testId="screen-consent" className="justify-between gap-6">
      <div className="flex flex-1 flex-col justify-center gap-8">
        <h1 className="wf-display text-center text-[38px] tracking-[0.06em] text-white">
          {t("consent.title")}
        </h1>
        ...
        <ul className="flex flex-col gap-5">
          {POINTS.map((key) => (
            <li key={key} className="flex items-start gap-4">
              ...
              />
              <span className="text-[22px] leading-[1.4] text-white/92">{t(key)}</span>
            </li>
          ))}
        </ul>

        <p className="rounded-2xl border border-white/12 bg-white/5 px-6 py-5 text-[18px] leading-[1.45] text-wf-dim">
          {t("consent.fiction")}
        </p>
      </div>
